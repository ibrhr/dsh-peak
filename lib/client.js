	...
	factory: (require) => {
		...
		*/
		function findHeaderMountTarget() {
			...
			for (const el of headerElements) if (el.children.length === 0 && el.textContent && el.textContent.toLowerCase().includes("mode")) {
				const modeContainer = el.closest("button, [class*=\"mode\"], [class*=\"badge\"], [data-slot*=\"mode\"], div") || el;
				if (modeContainer.parentElement) return {
					parent: modeContainer.parentElement,
					insertBeforeNode: modeContainer.nextSibling
				};
			}
			const modeSelector = document.querySelector("header :is([class*=\"mode\"], [data-slot*=\"mode\"], button:has([class*=\"mode\"]))");
			if (modeSelector && modeSelector.parentElement) return {
				parent: modeSelector.parentElement,
				insertBeforeNode: modeSelector.nextSibling
			};
			...
			if (header) return {
				parent: header.querySelector(":is([class*=\"row\"], [class*=\"bar\"], [class*=\"content\"], div)") || header,
